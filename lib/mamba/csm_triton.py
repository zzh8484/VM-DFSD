...
def cross_scan_fwd(x: torch.Tensor, in_channel_first=True, out_channel_first=True, scans=4):
    if in_channel_first:
        ...
        if scans==6:
            #s
            y = x.new_empty((B, 4, C, H * W))
            y[:, 0, :, :] =  reverse_odd_rows_optimized(x.clone()).flatten(2, 3)
            y[:, 1, :, :] = reverse_odd_rows_optimized(x.clone().transpose(2, 3)).flatten(2, 3)
            y[:, 2, :, :] =torch.flip(y[:, 0, :, :], dims=[-1])
            y[:, 3, :, :] =  torch.flip(y[:, 1, :, :], dims=[-1])
        if scans==7:
            # z,对
            y = x.new_empty((B, 8, C, H * W))
            y[:, 0, :, :] = x.flatten(2, 3)
            y[:, 1, :, :] = x.transpose(2, 3).flatten(2, 3)
            y[:, 2, :, :] = diagonal_gather_main(x)
            y[:, 3, :, :] = diagonal_gather_anti(x)
            #逆转
            y[:, 4, :, :] = torch.flip(y[:, 0, :, :], dims=[-1])
            y[:, 5, :, :] = torch.flip(y[:, 1, :, :], dims=[-1])
            y[:, 6, :, :] = torch.flip(y[:, 2, :, :], dims=[-1])
...
def cross_merge_fwd(y: torch.Tensor, in_channel_first=True, out_channel_first=True, scans=3):
    if out_channel_first:
        ...
        if scans == 0:
            ...
            y = y[:, 0:2] + y[:, 2:4].flip(dims=[-1]).view(B, 2, D, -1)
            y=reverse_odd_rows_optimized(y[:, 0].view(B, D, W, H)).view(B, D, -1) + reverse_odd_rows_optimized(y[:, 1].view(B, D, W, H)).transpose(2, 3).contiguous().view(B, D, -1)
        elif scans == 7:
            y = y[:, 0:4] + y[:, 4:8].flip(dims=[-1]).view(B, 4, D, -1)
            y = y[:, 0] + y[:, 1].view(B, D, W, H).transpose(2, 3).contiguous().view(B, D, -1)+diagonal_scatter_main(y[:, 2], (B,D,H,W)).contiguous().view(B, D, -1)+ diagonal_scatter_anti(y[:, 3], (B,D,H,W)).contiguous().view(B, D, -1)
        elif scans == 8:
            y = y[:, 0:4] + y[:, 4:8].flip(dims=[-1]).view(B, 4, D, -1)
            y = reverse_odd_rows_optimized(y[:, 0].view(B, D, W, H)).view(B, D, -1) + reverse_odd_rows_optimized(y[:, 1].view(B, D, W, H)).transpose(2, 3).contiguous().view(B, D, -1)+diagonal_scatter_main(y[:, 2], (B,D,H,W)).contiguous().view(B, D, -1)+ diagonal_scatter_anti(y[:, 3], (B,D,H,W)).contiguous().view(B, D, -1)
    if in_channel_first and (not out_channel_first):    
...
def diagonal_gather_anti(tensor):
    ...
    diag_id = (row + col)  # ↙方向：i + j

    flat_idx = diag_id.flatten()
    sort_order = flat_idx.argsort()  # 按 ↙方向对角线顺序排序

    tensor = tensor.view(B, C, -1)  # [B, C, H*W]
    tensor_sorted = tensor[:, :, sort_order]  # 排序后即为对角线方向
