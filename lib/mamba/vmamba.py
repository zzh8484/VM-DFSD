...
#         return F.conv2d(x, weight[:, :, None, None], bias)
...
class SS2Dde:
    def __initde__(
        ...
        **kwargs,    
        ...
        # forward_type debug =======================================
        FORWARD_TYPES = dict(
            v01=partial(self.forward_corede, force_fp32=(not self.disable_force32), selective_scan_backend="mamba", scan_force_torch=True),
            de=partial(self.forward_corede, force_fp32=False, no_einsum=True, scan_force_torch=True),  # selective_scan_backend="oflex", scan_mode="cross2d"
        )
    ...
    def forward_corede(
        ...
        **kwargs,
        ...
        if _scan_mode == -1:
            ...
            x_proj_bias = getattr(self, "x_proj_bias", None)
            xs = cross_scan_fn(x, in_channel_first=True, out_channel_first=True, scans=self.scan_mode, force_torch=scan_force_torch)
            if no_einsum:
            ...
            #_scan_mode
            y: torch.Tensor = cross_merge_fn(ys, in_channel_first=True, out_channel_first=True, scans=self.scan_mode, force_torch=scan_force_torch)
    ...
    def forwardde(self, x: torch.Tensor, **kwargs):
        short_x= x
        x = self.in_proj(x)
...
class SS2Dm0:
    def __initm0__(
        ...
        **kwargs,    
        ...
        # forward_type debug =======================================
        FORWARD_TYPES = dict(
            m0=partial(self.forward_corem1, force_fp32=False, dstate=d_state),
        )
        self.forward_core = FORWARD_TYPES.get(forward_type, None)
        k_group = 4
...
class SS2D(nn.Module, SS2Dv0, SS2Dv2, SS2Dv3, SS2Dm0,SS2Dde):
    def __init__(
        self,
        # basic dims ===========
        d_model=96,
        ...
        **kwargs,
        ...
        elif forward_type.startswith("de"):
            self.__initde__(**kwargs)
        else:
...
class Backbone_VSSM(VSSM):
    ...
    def load_pretrained(self, ckpt=None, key="model"):
        ...
        print(f"totalkeys: {len(_ckpt[key])}")
        incompatibleKeys = self.load_state_dict(_ckpt[key], strict=False)
        print(f"Missing keys: {len(incompatibleKeys.missing_keys)}")  
        print(f"Unexpected keys: {len(incompatibleKeys.unexpected_keys)}")   
...
if __name__ == "__main__":
    ...
    model_weights_path = '/root/haiguang/zzh/fin/pre/vssm_small_0229_ckpt_epoch_222.pth'
