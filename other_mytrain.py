...
def prepare_dataloader(batch_size=32,dataset=None,img_size=256):
    train_dataset = other_datasets(
...
def train(args=None):
    ...
    train_loader = prepare_dataloader(dataset=args.dataset,batch_size=args.batch_size,img_size=args.img_size)
    ...
    datset_name=args.dataset+'_'+args.name
    dir_name = os.path.join(dir, datset_name)
    ...
    test(name=args.name,model=model,dataset=args.dataset,results_dir=args.results_dir,img_size=args.img_size)
...
if __name__ == "__main__":
    ...
    #ThyroidNodule-TN3K
    #Breast-UDIAT
    parser.add_argument('--dataset', default='ThyroidNodule-DDTI',type=str)
